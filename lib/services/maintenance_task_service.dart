...
class MaintenanceTaskService {
  static final MaintenanceTaskService _instance = MaintenanceTaskService._internal();
  ...
  Future<List<MaintenanceTask>> getAllTasks({String? status}) async {
    ...
    try {
      ...
      final response = await _apiService.get('/aquariums/$_currentAquariumId/tasks$queryParam');
  ...
  Future<MaintenanceTask> updateTask(String taskId, MaintenanceTask task) async {
    if (_currentAquariumId == null) {
      throw Exception('Nessun acquario selezionato');
    }
  ...
  Future<MaintenanceTask> updateFrequency(String taskId, int frequencyDays) async {
    final tasks = await getAllTasks();
    final task = tasks.firstWhere((t) => t.id == taskId);
  ...
  Future<MaintenanceTask> updateReminder(String taskId, int hour, int minute) async {
    final tasks = await getAllTasks();
    final task = tasks.firstWhere((t) => t.id == taskId);
  ...
  Future<void> initializeDefaultTasks() async {
    ...
    final defaultTasks = MaintenanceTask.getDefaultTasks(_currentAquariumId.toString());
