//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:maintenance_service/api.dart';
import 'package:test/test.dart';

// tests for ProductDTO
void main() {
  // final instance = ProductDTO();

  group('test ProductDTO', () {
    // int id
    test('to test the property `id`', () async {
      // TODO
    });

    // String name
    test('to test the property `name`', () async {
      // TODO
    });

    // String category
    test('to test the property `category`', () async {
      // TODO
    });

    // String brand
    test('to test the property `brand`', () async {
      // TODO
    });

    // double quantity
    test('to test the property `quantity`', () async {
      // TODO
    });

    // String unit
    test('to test the property `unit`', () async {
      // TODO
    });

    // double cost
    test('to test the property `cost`', () async {
      // TODO
    });

    // String currency
    test('to test the property `currency`', () async {
      // TODO
    });

    // DateTime purchaseDate
    test('to test the property `purchaseDate`', () async {
      // TODO
    });

    // DateTime expiryDate
    test('to test the property `expiryDate`', () async {
      // TODO
    });

    // String notes
    test('to test the property `notes`', () async {
      // TODO
    });

    // String imageUrl
    test('to test the property `imageUrl`', () async {
      // TODO
    });

    // bool isFavorite
    test('to test the property `isFavorite`', () async {
      // TODO
    });

    // int usageFrequency
    test('to test the property `usageFrequency`', () async {
      // TODO
    });

    // DateTime lastUsed
    test('to test the property `lastUsed`', () async {
      // TODO
    });

    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // bool expired
    test('to test the property `expired`', () async {
      // TODO
    });

    // bool expiringSoon
    test('to test the property `expiringSoon`', () async {
      // TODO
    });

    // bool lowStock
    test('to test the property `lowStock`', () async {
      // TODO
    });

    // int daysSinceLastUse
    test('to test the property `daysSinceLastUse`', () async {
      // TODO
    });

    // bool shouldUseAgain
    test('to test the property `shouldUseAgain`', () async {
      // TODO
    });


  });

}
